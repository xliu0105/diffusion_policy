...
"""
    目前我看到这个DiffusionUnetHybridImagePolicy类和DiffusionUnetImagePolicy类的区别仅在于:
    对观测obs的encoder方法, DiffusionUnetHybridImagePolicy用的是robomimic中的配置方法以及obs_encoder, 而DiffusionUnetImagePolicy用的是自定义的obs_encoder
"""
...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
            ...
            crop_shape=(76, 76),  # 设置crop的大小，将原始图片裁剪成这个大小
            diffusion_step_embed_dim=256,  # 对timesteps进行位置编码的维度
            ...
            obs_encoder_group_norm=False,  # 是否将obs_encoder的batch norm进行替换为group norm
        ...
        # 获取并储存配置文件中，关于obs的信息，agent_pos的维度low_dim信息存在obs_config字典的low_dim中，rgb维度信息存在obs_config字典的rgb中
        ...
        obs_key_shapes = dict()
        for key, attr in obs_shape_meta.items():  # 遍历字典的一级元素
            shape = attr['shape']
            obs_key_shapes[key] = list(shape)

            type = attr.get('type', 'low_dim')  # .get方法，如果type不存在，则返回'low_dim'
            if type == 'rgb':
        ...
        # NOTE: 后面创建obs_encoder使用的是robomimic的encoder，是通过创建一整个algorithm对象，然后从中获取obs_encoder，其实可以直接创建obs_encoder，具体见robomimic的示例
        # init global state
        # ObsUtils 是一个用于处理观测数据的实用工具类，它通常提供一系列方法来初始化、格式化、和预处理来自环境或数据集的观测数据
        # initialize_obs_utils_with_config中还会调用robomimic的initialize_obs_utils_with_obs_specs
        ObsUtils.initialize_obs_utils_with_config(config)  # 根据config中的obs_config，初始化哪些obs_key是low_dim，哪些是rgb

        # load model
        # 创建一个robomimic的策略对象，包含了算法网络和配置信息
        ...
        obs_encoder = policy.nets['policy'].nets['encoder'].nets['obs']  # 使用robomimic的obs编码网络
        
        if obs_encoder_group_norm:
            # replace batch norm with group norm，将obs_encoder中的batch norm替换为group norm
            replace_submodules(
        ...
        # obs_encoder.obs_randomizers['agentview_image']，将obs_encoder中的CropRandomizer替换为自定义的CropRandomizer
        if eval_fixed_crop:
            replace_submodules(
                root_module=obs_encoder,
